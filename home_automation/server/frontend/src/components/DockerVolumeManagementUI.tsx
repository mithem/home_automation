...
export default class DockerManagmentUI extends React.Component<{}, {volumes: DockerVolumeData[], error?: Error, loading: boolean}> {
	...
	render() {
		...
		const noVolumesWarning = this.state.volumes.length === 0 ? (
			<Alert variant="info">
				You currently don't have any docker volumes. Create some (e.g. by creating a container with one) to view them here.
			</Alert>
		) : null
